    ...
    <div className="min-h-screen p-8 flex flex-col">
      <div className="container mx-auto max-w-6xl flex-grow grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div>
          ...
          <p className="text-lg mb-8">Local Runtime Dashboard</p>
          ...
          <div className="border rounded-xl p-6 ">
            ...
            {error && <div className="p-4 rounded-lg mb-6 font-mono text-sm">Erro: {error}</div>}
                ...
                {workflows.length === 0 ? (
                  ...
                  workflows.map((wf, i) => (
                    <div key={i} className="rounded-lg p-4">
                      <h3 className="text-lg font-medium mb-1">{wf.name || "Unnamed Workflow"}</h3>
                      ...
                      <div className="flex flex-col gap-2">
                        {wf.jobs?.map((job: Job, j: number) => (
                          <div
                            key={j}
                            className="flex items-center justify-between pl-3 pr-2 py-2 rounded border border-white/5 hover:border-indigo-500/30 transition-colors"
                          >
        ...
        <div className="flex flex-col rounded-xl overflow-hidden border h-[80vh] sticky top-8">
          <div className="bg-card px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="ml-2 text-sm font-medium text-gray-400 font-mono">
                gravity-terminal
              </span>
            </div>
            {isRunning && (
              <div className="flex items-center gap-4">
                <Button onClick={stopJob} variant="destructive">
                  Stop
                </Button>
          ...
          <div className="flex-1 p-4 overflow-y-auto font-mono text-xs leading-relaxed">
            {logs.length === 0 ? (
              ...
              logs.map((log) => {
                ...
                return (
                  <div key={log.id} className={`whitespace-pre-wrap ${color}`}>
                    {log.text}
                  </div>
                );
              })
